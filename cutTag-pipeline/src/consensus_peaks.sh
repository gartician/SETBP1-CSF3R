...
cat ${mpks[@]} | sort -k1,1 -k2,2n \
    | bedtools merge -d -1 \
    | bedtools intersect -c -a - -b ${mpks[@]} \
    | awk -v OFS='\t' '$4 >= 2' | cut -f1-3 > data/counts/${MARK}_consensus.bed
...
ls ${BAMDIR}/*${MARK}*.bam | sed 's!.*/!!' | cut -d. -f1 | cut -d_ -f1-2 | xargs | tr ' ' '\t' | awk '{print "chrom\tstart\tend\t" $0}' | cat - ${OUTFILE}_tmp > ${OUTFILE}
